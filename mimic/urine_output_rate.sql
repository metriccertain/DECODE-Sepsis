    ...
    SELECT tm.stay_id
        , CASE
            WHEN LAG(charttime) OVER w IS NULL
                ...
                THEN ( UNIXEPOCH(charttime) - UNIXEPOCH(intime_hr) ) / 60
            --ELSE DATETIME_DIFF(charttime, LAG(charttime) OVER w, MINUTE)
            ELSE ( UNIXEPOCH(charttime) = UNIXEPOCH(LAG(charttime) OVER w) ) / 60
...
, ur_stg AS (
    SELECT io.stay_id, io.charttime
        ...
        , SUM(CASE WHEN UNIXEPOCH(io.charttime) - UNIXEPOCH(iosum.charttime) <= 5*3600
        ...
        , SUM(CASE WHEN UNIXEPOCH(io.charttime) - UNIXEPOCH(iosum.charttime) <= 11*3600
            THEN iosum.urineoutput
    ...
    LEFT JOIN uo_tm iosum
        ON io.stay_id = iosum.stay_id
            ...
            AND io.charttime <= (
                ...
                DATETIME(iosum.charttime, '23 HOURS')
            )
    ...
    , CASE
        WHEN
            uo_tm_6hr >= 6 THEN ROUND(
                CAST((ur.urineoutput_6hr / wd.weight / uo_tm_6hr) AS NUMERIC)
                , 4
            )
