...
mimic_icu.chartevents <- mimic_icu.chartevents %>%
  inner_join(
    distinct(dplyr::select(`physionet-data.mimic_icu.icustays`, stay_id, intime)), 'stay_id'
  ) %>%
  filter(
    as.POSIXct(intime) - 24*3600 <= as.POSIXct(charttime) & as.POSIXct(charttime) <= as.POSIXct(intime) + 36*3600
  ) %>%
  dplyr::select(
    -intime
  )
...
`physionet-data.mimic_derived.icustay_hourly` <- `physionet-data.mimic_derived.icustay_times` %>%
  ...
  rowwise() %>%
  mutate(
    zendtime = list(start_dttm - 24*3600 + seq(0, 48*3600, by = 3600))
  ) %>%
  unnest(zendtime) %>%
  ungroup() %>% 
  mutate(
...
out_tb <- out_tb %>%
  inner_join(
    dplyr::select(`physionet-data.mimic_icu.icustays`, subject_id, hadm_id, stay_id), c('subject_id', 'hadm_id')
  ); dim(out_tb)
...
out_tb <- out_tb %>%
  mutate(
    icu_days = round(as.numeric(difftime(outtime, intime_hr, units='days')), digits=3)
  ) %>%
  dplyr::select(
    -outtime_hr
  ); dim(out_tb)
...
out_tb <- out_tb %>%
  mutate(
    susp = if_else(as.POSIXct(admittime) - 24*3600 <= suspected_infection_time & suspected_infection_time <= as.POSIXct(intime_hr) + 24*3600 &
...
out_tb <- out_tb %>%
  left_join(
    filter(tmp, prior_flag == 1), c('subject_id','intime')
  ) %>%
  dplyr::select(
    -prior_flag
  )
...
out_tb <- out_tb %>%
  left_join(
    filter(tmp, prior_flag == 0), c('subject_id','intime')
  ) %>%
  dplyr::select(
    -prior_flag
  )
...
tmp <- tmp %>%
  inner_join(
    dplyr::select(out_tb, subject_id, hadm_id, stay_id, admittime, dischtime), 'subject_id', relationship='many-to-many'
  )

tmp <- tmp %>%
  filter(
    admittime <= charttime & charttime <= dischtime
  ) %>%
...
ringers_lactate <- `physionet-data.mimic_icu.inputevents` %>% 
  filter(
    itemid %in% c(225827, 225828)
  ) %>%
  dplyr::select(
    subject_id, starttime, endtime, amount, totalamount, ordercategoryname,
