...
demo <- demo %>%
  mutate(
    ...
    pneumoniadttm = if_else(pneumoniadttm == '', NA, pneumoniadttm),
...
demo <- demo %>%
  ...
  dplyr::select(
    ...
    metastatic_cancer_hist, aids_hist, hosp_death, prlos_death, pneumoniadttm
  )
```
...
qdemo <- demo %>%
  mutate(
    start_dttm = intime - minutes(minute(intime)) - seconds(second(intime))
  ) %>%
  rowwise() %>%
  mutate(subject_id, intime, outtime, dttm = list(seq(start_dttm, start_dttm + hours(48), by = '15 mins'))) %>%
  unnest(dttm) %>%
  ungroup() %>%
  filter(
    intime <= dttm & dttm <= outtime & dttm <= as_datetime(intime) + hours(25)
  ) %>%
  dplyr::select(
...
vitals <- vitals %>%
  mutate(
    charttime = as_datetime(charttime)
  )
```
...
vitals <- vitals %>%
  filter(
    intime - hours(6) <= charttime & charttime <= intime + hours(26)
  )
```
...
labs <- labs %>%
  dplyr::select(
    subject_id, charttime, glucose:ast
  ) %>%
  distinct(); dim(labs)
```
...
labs <- labs %>%
  inner_join(tmp, join_by(subject_id), relationship='many-to-many')

labs <- labs %>%
  filter(
    as_datetime(intime) - hours(6) <= charttime & charttime <= as_datetime(intime) + hours(25)
  )
```
...
qdemo <- qdemo %>%
  left_join(tmp, join_by(subject_id, intime, dttm)); dim(qdemo)

qdemo <- qdemo %>%
  left_join(labs1, join_by(subject_id, intime, dttm)); dim(qdemo)
...
```{r cpn}
cpn <- read.csv(file='./derived/v4/cpn.csv', nrows=row_limit)
...
```{r ringers_lactate}
ringers_lactate <- read.csv('./derived/v4/ringers_lactate.csv', nrows=row_limit)

ringers_lactate <- ringers_lactate %>%
  mutate(
    starttime = as_datetime(starttime), 
    endtime = as_datetime(endtime)
  )
...
tmp <- ringers_lactate %>%
  mutate(
    endtime = case_when(
      ordercategorydescription != 'Continuous IV' ~ starttime + hours(1),
      TRUE ~ endtime
    )
  )
...
tmp <- tmp %>%
  mutate(
    fluid_rate = 0.25 * round(amount / as.numeric(difftime(endtime, starttime, units='hours')), digits=3)
  )
...
if (params$write_data_mode == 'write') {
  write.csv(qdemo, file='./derived/v4/qdemo.csv', row.names=FALSE, na='')
}
```
...
qdemo <- qdemo %>%
  ...
  mutate(
    across(
      c(peep, tidal_vol, map, hr, fio2, spo2, temp, rr, lglucose, hgb, lpotassium, sodium, lplatelet,
         hct, ph, lcreatinine, lwbc, lbun, hco3, lpaco2, lpao2, llactate,
          linr, anion_gap, last, gcs), ~vctrs::vec_fill_missing(.x, direction = 'down', max_fill = 23)
    )
  ) %>%
  ungroup()

# join fio2 (only observed values), tidal volumes and peep (both set to 0 when non-ventilated)
qdemo <- qdemo %>%
...
qdemo_new <- qdemo %>%
  dplyr::select(
    ...
    sodium, lplatelet, hct, ph, lcreatinine, lwbc, lbun, hco3, lpaco2, lpao2, llactate, linr, anion_gap, last, gcs, 
    ecmo, cpn, NOREPINEPHRINE, EPINEPHRINE, DOBUTAMINE, VASOPRESSIN, lactring, pneumonia, 
