...
# old data
qdemo_old_scaled <- read.csv(file='./derived/v3/qdemo_scaled.csv') %>%
  mutate(
    dttm = as.POSIXct(dttm)
  ) %>%
  arrange(admit_id, row_id)

comp <- comparedf(x=qdemo_old_scaled, y=qdemo_new_scaled, by = c('admit_id','row_id'), int.as.num = TRUE)
...
# old data
qdemo_old_scaled <- read.csv(file='./derived/v3/qdemo_lvcf_scaled.csv') %>%
  mutate(
    dttm = as.POSIXct(dttm)
  ) %>%
  arrange(admit_id, row_id)
