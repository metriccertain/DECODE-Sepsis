...
qdemo <- qdemo %>%
  left_join(tmp, join_by(id, row_id))
```
...
# labs prior to admission, already filtered to within 6 hours
labs1 <- labs %>%
...
labs1 <- labs1 %>%
  pivot_wider(id_cols = subject_id:row_id, names_from = chartname, values_from = chartvalue)
