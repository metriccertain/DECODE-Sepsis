...
, vd1 AS (
    SELECT
        ...
        , (STRFTIME('%s', charttime) - STRFTIME('%s', charttime_lag)) / 3600 AS ventduration
        ...
        , CASE
            ...
            WHEN ((STRFTIME('%s', charttime) - STRFTIME('%s', charttime_lag)) / 3600) >= 14 THEN 1
    ...
    , MAX(
        CASE
            WHEN charttime_lead IS NULL
                ...
                OR ((STRFTIME('%s', charttime_lead) - STRFTIME('%s', charttime)) / 3600) >= 14
                THEN charttime
            ELSE charttime_lead
        END
    ) AS endtime
