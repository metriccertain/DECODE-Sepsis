...
WITH co AS (
    SELECT
        ...
        , stay_id
        , intime AS starttime
        , DATETIME(intime, '24 HOURS') AS endtime
...
, cpap AS (
    SELECT
        ...
        , co.stay_id
        -- , GREATEST(
        --    MIN(DATETIME(charttime, '1 HOUR')), co.starttime
        -- ) AS starttime
        , CASE
              WHEN MIN(DATETIME(ce.charttime, '+1 hour')) < co.starttime THEN co.starttime
              ELSE MIN(DATETIME(ce.charttime, '+1 hour'))
          END as starttime
        -- , LEAST(
        --    MAX(DATETIME(charttime, '4 HOURS')), co.endtime
        -- ) AS endtime
        , CASE
              WHEN MAX(DATETIME(ce.charttime, '+4 hour')) < co.endtime THEN MAX(DATETIME(ce.charttime, '+4 hour'))
              ELSE co.endtime
          END AS endtime
        , MAX(
            CASE
                WHEN
                    LOWER(ce.value) LIKE '%cpap mask%' OR LOWER(ce.value) LIKE '%bipap%' THEN 1
    ...
    WHERE ce.itemid = 226732
        ...
        AND LOWER(ce.value) LIKE '%cpap mask%' OR LOWER(ce.value) LIKE '%bipap%'
...
SELECT s.subject_id, s.hadm_id, s.stay_id
    ...
    , sapsii
    , 1 / (
        1 + EXP(- (-7.7631 + 0.0737 * (sapsii) + 0.9971 * (log(sapsii + 1))))
    ) AS sapsii_prob
    , age_score
    , hr_score
    , sysbp_score
